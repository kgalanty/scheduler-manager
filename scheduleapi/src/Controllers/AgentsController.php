...
class AgentsController
{
  ...
  public function verifyAgent($request, $response, $args)
  {
    ...
    if (
      DB::table('tbladmins as a')->join('tbladminperms as ap', function ($q) {
        $q->on('ap.roleid', '=', 'a.roleid')->where('ap.permid', '=', AgentConstants::EDITOR_PERMISSION);
      })
      ->where('a.id', $author)->count() == 0
    ) {
