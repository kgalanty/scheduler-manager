 ...
 public function assignToShift($request, $response, $args)
   ...
   $data = $request->getParsedBody();
    ...
    public function shiftsGroups($request, $response, $args)
    {
        ...
        {
            $days[$t->shift_id][$t->day][] = ['id' => $t->id, 'agent' => $t->firstname.' '.$t->lastname, 'color' => $t->color??'#000', 'bg' => $t->bg??'rgb(202 202 202)', 'author' => $t->author, 'draft'=>$t->draft, 'deldraftauthor' => $t->draftauthor??false,
                'shift'=>$t->from.':'.$t->to];
        }
        $data = ['shifts' => $shifts, 't' => $days, 'group' => $group];
